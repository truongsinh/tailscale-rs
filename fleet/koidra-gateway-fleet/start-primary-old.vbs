' start-primary-old.vbs — ROLLBACK: detached relaunch of the OLD dir's primary channel.
'
' When the new primary channel fails the coordinator's external validation gate,
...
If fso.FileExists(oldDir & "\run-node2.cmd") Then
    cmd = "cmd /c """ & oldDir & "\run-node2.cmd"" primary 22"
ElseIf fso.FileExists(oldDir & "\supervisor.vbs") Then
    cmd = wscriptExe & " """ & oldDir & "\supervisor.vbs"" primary 22"
